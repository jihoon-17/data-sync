...
/**
 *读取mac数据类
 * 存到data-for-mysql kafka topic中
 * @author zhixun
 */
@Slf4j
@Service("read4MysqlDBService")
public class Read4MysqlDBServiceImpI implements ReaderService {
    ...
    @Override
    public void read(String targetSchema, String tablename) throws UnsupportedEncodingException {
        ...
        try (CloseableHttpClient httpClient = HttpUtil.createCustomPoolingHttpClient()){
            ...
            while (!isQueryLimitEmpty){
                ...
                if(nonEmpty){
                    ...
                    log.info("Read4MysqlDBServiceImpI done:{},records size:{}",done,j.length());
                    if(!done){//循环获取下一页的数据直到done为true,说明已获取到最新的数据
                        nextRecordsUrl = json.getString("nextRecordsUrl");
                        log.info("Read4MysqlDBServiceImpI nextRecordsUrl:{}",nextRecordsUrl);
                ...
                log.info("Read4MysqlDBServiceImpI maxLastModityDate:{},nonEmpty:{}",maxLastModityDate,nonEmpty);
                //生产者: 数据发送到kafka
                if(nonEmpty){
                    Producer.send(Constants.MYSQL_TOPIC, JSONUtil.toJsonStr(new DataVo(j.toString(), querySQL, config.getTargettablename())));
                    log.info("send data to kafka data-for-mysql topic success,data size:{}",j.length());
                }
            ...
            log.error("Read4MysqlDBServiceImpI error:",e);
        }
